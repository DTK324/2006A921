    ...
    def train_model(self, model_name, param_widgets):
        if model_name == "Linear Regression":
            ...
            if loss_fn == "MSE":
                ...
                delta = 1.0  # İsteğe bağlı olarak GUI’den alınabilir
                residual = self.y_test - y_pred
                loss_value = np.where(np.abs(residual) <= delta,
                                  0.5 * residual ** 2,
                                  delta * (np.abs(residual) - 0.5 * delta))
                loss_value = np.mean(loss_value)
        
            # Modeli kaydet ve GUI’ye göster
            ...
            # Cross-Entropy ve Hinge loss hesapla
